    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            ...
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                ...
                focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">{{ __('Add user') }}</a>
            </div>
        </div>
                    ...
                    <div class="shadow-sm ring-1 ring-black ring-opacity-5">
                        <table class="min-w-full border-separate" style="border-spacing: 0">
                            <thead class="bg-gray-50">
                                ...
                                backdrop-blur backdrop-filter sm:pl-6 lg:pl-8">{{ __('Name') }}</th>
                                ...
                                backdrop-filter sm:table-cell">{{ __('Email') }}</th>
                                    <th scope="col" class="sticky top-16  z-10 hidden border-b border-gray-300 bg-gray-50
                                bg-opacity-75 px-3 py-3.5 text-left text-sm font-semibold text-gray-900 backdrop-blur
                                backdrop-filter lg:table-cell">{{ __('Plan') }}</th>
                                    <th scope="col" class="sticky top-16  z-10 border-b border-gray-300 bg-gray-50
                                bg-opacity-75 px-3 py-3.5 text-left text-sm font-semibold text-gray-900 backdrop-blur
                                backdrop-filter">{{ __('Organization') }}</th>
                            ...
                            <tbody class="bg-white">
                                ...
                                <tr>
                                    ...
                                    text-right text-sm font-medium sm:pr-6 lg:pr-8">
                                        @role('super admin')
                                        <a href="{{ route('admin.users.edit', $subscription->user) }}">{{ __('View status') }}</a>
                                        @endrole
                                ...
                                <tr>
                                    ...
                                    text-right text-sm font-medium sm:pr-6 lg:pr-8">
                                        @role('super admin')
                                        <a href="{{ route('admin.users.edit', $muser) }}">{{ __('View status') }}</a>
                                        @endrole
                                        @hasanyrole('emiweb admin|emiweb staff')
                                        <a href="{{ route('emiweb.users.edit', $muser) }}" class="text-indigo-600 hover:text-indigo-900">{{ __('View Status') }}</a>
                                        @endhasanyrole
