        ...
        <aside class="py-6 px-2 sm:px-6 lg:py-0 lg:px-0 lg:col-span-3">
            <nav class="space-y-1 lg:fixed">
                ...
                <a href="#organization_detail" class="bg-gray-50 text-indigo-700 hover:text-indigo-700 hover:bg-white group rounded-md px-3 py-2 flex items-center text-sm font-medium" aria-current="page">
                    ...
                    <span class="truncate">{{ __('Organization Details') }} </span>
                </a>
                ...
                <a href="#archive_access" class="text-gray-900 hover:text-gray-900 hover:bg-gray-50 group rounded-md px-3 py-2 flex items-center text-sm font-medium">
                    ...
                    <span class="truncate">{{ __('Archives') }} </span>
                </a>
        ...
        <div class="space-y-6 sm:px-6 lg:px-0 lg:col-span-9">
            ...
            <div class="shadow sm:rounded-md sm:overflow-hidden" id="organization_detail">
                <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
                    <div>
                        <h3 class="text-lg leading-6 font-medium text-gray-900">{{ __('Organization Details') }}</h3>
                        <p class="mt-1 text-sm text-gray-500">{{ __('These are basic details of your organization.') }}</p>
                    </div>
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                            <label for="ip_address" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
                                {{ __('IP address(comma separated)') }} </label>
                            <div class="mt-1 sm:mt-0 sm:col-span-2">
                                ...
                                <div id="tags">
                                    ...
                                    @endforeach
                                    @endif
                                    <input type="text" value="" id="ip_add" placeholder="{{ __('Add New IP Address') }}" />
                                </div>
                ...
                <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                    <button type="submit" class="bg-indigo-600 border border-transparent rounded-md shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">{{ __('Save') }}</button>
                </div>
            </div>
            {!! Form::close() !!}
            ...
            <div class="shadow sm:rounded-md sm:overflow-hidden" id="archive_access">
                <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
                    <div class="md:grid md:grid-cols-3 md:gap-6">
                        <div class="md:col-span-1">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ __('Archive Access') }}</h3>
                            <p class="mt-1 text-sm text-gray-500">{{ __('Select which archive this organization should have.') }}</p>
                        </div>
                        <div class="mt-5 md:mt-0 md:col-span-2">
                            <fieldset>
                                <div class="flex items-start">
                                    <div class="h-5 flex items-center">
                                        <input class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" checked id="selall" type="checkbox">
                                    </div>
                                    <div class="ml-3 text-sm">
                                        <label class=" font-medium text-gray-700" id="lb">{{ __('Select all') }}</label>
                                    </div>
                                    <script>
                                        function toggleCheck(id, className, label) {
                                            const selall = document.getElementById(id);
                                            const labelid = document.getElementById(label);
                                            // Fäst klickhändelse på förälderelementet
                                            selall.addEventListener('click', function(event) {
                                                // Kontrollera om det klickade elementet har klassen 'on'

                                                // Hämta alla element med klassen 'on'
                                                const elements = document.querySelectorAll('.' + className);
                                                // Loopa igenom och ändra checked-egenskapen för varje element
                                                elements.forEach(function(element) {
                                                    if (!selall.checked) {
                                                        element.checked = false;
                                                        labelid.innerText = "{{ __('Select all') }}";
                                                    } else {
                                                        element.checked = true;
                                                        labelid.innerText = "{{ __('Select not all') }}";
                                                    }
                                ...
                                <div class="mt-4 space-y-4">
                                    <div class="flex items-start">
                                        <div class="h-5 flex items-center">
                                            {!! Form::checkbox('archive_id[]',
                                            $archive->id,
                                            in_array($archive->id, $TheOrganization->archives->pluck('id')->toArray()) ? 'checked' : '',
                                            [
                                            'class' => 'focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded ' . (in_array($archive->id, $TheOrganization->archives->pluck('id')->toArray()) ? 'on' : ''),
                                            'id' => 'archive-id-' . $archive->id
                                            ])
                                            !!}
                                        </div>
                                        <div class="ml-3 text-sm">
                ...
                <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                    <a href="{{ route('admin.organizations.index') }}" type="button" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm
                        text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">{{ __('Cancel') }}</a>
            ...
            <div class="shadow sm:rounded-md sm:overflow-hidden" id="associated_users">
                <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">{{ __('Users associated with this organization.') }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <label for="email" class="block text-sm font-medium text-gray-700">
                            {{ __('Search user to add with their email') }}
                        </label>
                        <div class="mt-1 flex rounded-md shadow-sm">
                            <div class="relative flex items-stretch flex-grow focus-within:z-10">
                                ...
                                <input type="email" name="email" id="email" class="focus:ring-indigo-500
                                            ...
                                            sm:text-sm border-gray-300" placeholder="{{ __('example@example.com') }}">
                            </div>
                            <button type="submit" class="-ml-px relative inline-flex items-center space-x-2
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 16l2.879-2.879m0 0a3 3 0 104.243-4.242 3 3 0 00-4.243
                                ...
                                <div class="flex gap-4">
                                    ...
                                    <button type="submit" class="inline-flex items-center px-3 py-2 border
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" class="-ml-0.5 mr-2 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M20.618 5.984A11.955 11.955 0 0112 2.944a11.955 11.955
