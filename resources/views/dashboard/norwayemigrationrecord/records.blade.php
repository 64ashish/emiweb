        ...
        <section class="pt-6" aria-labelledby="section-1-title">
            <div class="bg-white py-6 pl-4 pr-3 border-gray-300 shadow md:rounded-lg">
                <p class="text-left text-sm font-semibold text-gray-900 pb-4">
                    {{ __('Advanced search') }}: Emigranter i norska kyrkböcker
                </p>

                @if(auth()->user()->hasRole('regular user|subscriber'))
                    @if(isset($keywords))
                        {!! Form::model($keywords,['route' => 'nerc.search'])  !!}
                    @endif
                    @if(!isset($keywords))
                        {!! Form::open(['route' => 'nerc.search'])  !!}
                    @endif
                            ...
                            @foreach($records as $record)
                                <tr  class="odd:bg-white even:bg-gray-100">
                                    ...
                                    <td class="whitespace-nowrap border-b border-gray-200 px-3 py-4 text-sm
                                                                text-gray-500 hidden lg:table-cell">{{ $record->sex }}</td>
                                    <td class="whitespace-nowrap border-b border-gray-200 px-3 py-4 text-sm
                                                                text-gray-500 hidden sm:table-cell max-w-xs truncate ...">
                                        {{ $record->from_location}}, {{ $record->from_region }}</td>
                                    <td class="whitespace-nowrap border-b border-gray-200 px-3 py-4 text-sm
                                                                text-gray-500 hidden lg:table-cell">
                                        {{ $record->profession }}</td>
                                    <td class="whitespace-nowrap border-b border-gray-200 px-3 py-4 text-sm
                                                                text-gray-500">
                                        <p class="max-w-xs truncate ...">{{ $record->destination_location }}
                                            {{ $record->destination_area }}  {{ $record->destination_country }}</p>
                                    </td>
                                    <td class="whitespace-nowrap border-b border-gray-200 px-3 py-4 text-sm
                                                                text-gray-500 ">
