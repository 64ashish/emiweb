<x-app-layout>
    ...
    <div class="flex gap-x-5">
        ...
        <div class="con-col-3 gap-x-5">
            <div class="lg:col-span-1 bg-white shadow overflow-hidden sm:rounded-lg mb-6">
                <div class="px-4 py-5 sm:px-6">
                    <h3 class="text-lg leading-6 font-medium text-gray-900">{{ sprintf(__('User information for %s'),$user->name) }}</h3>
                    <p class="mt-1 max-w-2xl text-sm text-gray-500">{{ __('These are the user details for your account') }}</p>
                </div>
                <div class="border-t border-gray-200 px-4 py-5 sm:p-0">


                    <div class="grid grid-cols-2 gap-x-5 sm:grid-cols-2">
                        {!! Form::model($user,['route' =>['home.users.update',$user], 'method' => 'put', 'class'=>'col-span-1']) !!}
                        <div>
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-1 sm:gap-4 sm:px-6 text-sm font-medium">{{ __('Note: That if you change your e-mail, you must log in with new e-mail. if you have accidentally forgotten or have problems logging in, you can contact info@emiweb.se for help.') }}</div>
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                <dt class="text-sm font-medium text-gray-500 flex items-center">{{ __('Email') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    <input type="email" class="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md" value="{{ $user->email }}">
                                </dd>
                            </div>
                            ...
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-1 sm:gap-4 sm:px-6 text-sm font-medium">{{ __('We encourage you to use a strong password with numbers and characters') }}</div>
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">

                                <dt class="text-sm font-medium text-gray-500 flex items-center">{{ __('Current password') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                ...
                                <dt class="text-sm font-medium text-gray-500 flex items-center">{{ __('New password') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <div class="px-4 py-6 text-right sm:px-6">
                             ...
                             focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">{{ __('Save') }}</button>
                            </div>
                            {!! Form::close() !!}
                        </div>
                        <div class="col-span-1 ">
                            {!! Form::model($user,['route' =>['home.users.update',$user], 'method' => 'put']) !!}
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-1 sm:gap-4 sm:px-6 text-sm font-medium">{{ __('Should you wish for us to issue an invoice, please fill out the specified details below. Otherwise, it is not mandatory to do so. Once completed, kindly send us an email, and we will take care of the rest.') }}</div>
                            ...
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
                                <dt class="text-sm font-medium text-gray-500 flex items-center">{{ __('Country') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
                                <dt class="text-sm font-medium text-gray-500 flex items-center">{{ __('County') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 flex items-center">
                                <dt class="text-sm font-medium text-gray-500 flex items-center">{{ __('City') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                <dt class="text-sm font-medium text-gray-500 flex items-center">{{ __('Address') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dt class="text-sm font-medium text-gray-500 flex items-center">{{ __('Post Code') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
            ...
            <div class="col-span-2 lg:col-span-1 bg-white shadow overflow-hidden sm:rounded-lg">
                ...
                <div class="px-4 sm:px-6 mb-6">
                    <form action="/subscribe/{{ $user->subscriptions()->active()->first()->id }}/update" method="post" id="payment-form" data-secret="{{ $intent->client_secret }}">
                        @csrf
                        <div class="form-row">
                            <div x-data="{ value: '{{ $user->subscriptions()->active()->first()->stripe_price }}' }" class="md:flex md:justify-center md:space-x-6 my-8 ">
                                <div class="flex items-center space-x-3 rounded-md shadow-lg px-3 lg:px-6" x-bind:class="value == '{{ config('services.subscription.3_months') }}' ? 'border border-solid border-indigo-500 shadow-indigo-300/50' : 'shadow-gray-300/50 border border-solid '">
                                    <label for="standard" class="flex items-center gap-2">
                                        <input type="radio" name="plan" id="standard" value="{{ config('services.subscription.3_months') }}" x-model="value" checked="{{ $user->subscriptions()->active()->first()->stripe_price == config('services.subscription.3_months') ? 'checked':'' }}">
                                ...
                                <div class="flex items-center space-x-3 rounded-md shadow-lg px-3 lg:px-6" x-bind:class="value == '{{ config('services.subscription.1_year') }}' ? 'border border-solid border-indigo-500 shadow-indigo-300/50' : 'shadow-gray-300/50 border border-solid '">
                                    <label for="premium" class="flex items-center gap-2">
                                        <input type="radio" name="plan" id="premium" value="{{ config('services.subscription.1_year') }}" x-model="value" checked="{{ $user->subscriptions()->active()->first()->stripe_price == config('services.subscription.1_year')?'checked':'' }}">
                ...
                <div class="border-t border-gray-200  px-4 py-5 ">
                    ...
                    {{ sprintf(__('Your subscription ends on %s'),  $user->subscriptions()->active()->first()->ends_at->format('Y.m.d')) }}
                    @else
                ...
                <div class="px-4 sm:px-6">
                    ...
                    <form action="/subscribe" method="post" id="payment-form" data-secret="{{ $intent->client_secret }}">
                        @csrf
                        <div class="form-row grid grid-cols-2 gap-x-5 sm:grid-cols-2">
                            <div x-data="{value:null}" class="flex-row-reverse  md:justify-center my-8">
                                <div class="flex items-center  rounded-md shadow-lg px-3 lg:px-6" x-bind:class="value == '{{ config('services.subscription.3_months') }}' ? 'border border-solid border-indigo-500 shadow-indigo-300/50' : 'shadow-gray-300/50 border border-solid '">
                                    <label for="standard" class="flex items-center gap-2">
                                        <input type="radio" name="plan" id="standard" value="{{ config('services.subscription.3_months') }}" onclick="getPlanId('{{ config('services.subscription.3_months') }}')" x-model="value" checked="">
                                        <div class="font-bold text-gray-900 pr-6 flex flex-col justify-center">
                                            <div class="mt-6">
                                                <span class="text-2xl lg:text-3xl">200</span>
                                                <span class="text-lg lg:text-xl">SEK</span>
                                ...
                                <div class="flex items-center  rounded-md shadow-lg px-3 mt-8 lg:px-6" x-bind:class="value == '{{ config('services.subscription.1_year') }}' ? 'border border-solid border-indigo-500 shadow-indigo-300/50' : 'shadow-gray-300/50 border border-solid '">
                                    <label for="premium" class="flex items-center gap-2">
                                        <input type="radio" name="plan" id="premium" value="{{ config('services.subscription.1_year') }}" onclick="getPlanId('{{ config('services.subscription.1_year') }}')" x-model="value">
                                        <div class="font-bold text-gray-900 pr-6  flex flex-col">
                                            <div class="mt-6">
                                                <span class="text-2xl lg:text-3xl">600</span>
                                                <span class="text-lg lg:text-xl">SEK</span>
                            ...
                            <div class="col-span-1">
                                ...
                                <div class="flex flex-col pl-8  pr-4 mb-5 pr-8">
                                    <label for="cardholder-name" class="pt-8 py-2 px-3  text-gray-500 font-medium">{{ __('Discount coupon') }}</label>
                                    <div class="flex box-sizing gap-4">
                                        <input type="text" id="coupon" class="placeholder:text-gray-500 block w-full border box-sizing border-gray-300 rounded-md py-2 pr-4 px-1 shadow-sm focus:outline-none focus:border-indigo-500 focus:placeholder:text-transparent focus:ring-1 sm:text-sm" value="">
                                ...
                                <div class="flex flex-col pl-8 pr-8 mb-5">
                                    <label for="cardholder-name" class="py-2 px-3  text-gray-500 font-medium">{{ __('Cardholder\'s Name') }}</label>
                                    <input type="text" id="cardholder-name" class="placeholder:text-gray-500 block  border border-gray-300 rounded-md py-2
                                    px-3 shadow-sm focus:outline-none focus:border-indigo-500 focus:placeholder:text-transparent focus:ring-1 sm:text-sm box-sizing">
                                </div>
